...
export function buildAdReportAnalysisPrompt(
  ...
  lines.push("");
  lines.push(
    "1部目（見出し不要）: 3〜6文程度で、前月比較・キャンペーン分析・成長データとの関連（データがあれば）を踏まえた状況分析を書く。" +
      "良かった点・気になる点の両方に触れる。"
  );
  lines.push("");
  lines.push(
    '2部目: 1行目に "## 提案" とだけ書いた見出しを置き、その後に2〜4文程度で具体的な改善提案を書く。' +
      "「継続すべきこと」「改善すべきこと」「中止すべきこと」「次に試すべきこと」のうち、データから妥当なものを1〜2個だけ選んで簡潔に述べる。" +
      "抽象論ではなく、キャンペーン名や年齢層など具体的な固有名詞を使うこと。文末は「〜はいかがでしょうか」「〜をおすすめいたします」" +
      "「ぜひご検討ください」のような依頼・提案調にし、命令調は使わないこと。"
  );
