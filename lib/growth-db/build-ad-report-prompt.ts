...
export function buildAdReportAnalysisPrompt(
  ...
  lines.push("");
  lines.push(
    "1部目（見出し不要）: 3〜5文・180字以内で、前月比較・キャンペーン分析・成長データとの関連（データがあれば）を踏まえた状況分析を書く。" +
      "良かった点・気になる点の両方に触れる。180字を超えそうな場合は文を削って必ず収めること。"
  );
  lines.push("");
  lines.push(
    '2部目: 1行目に "## 提案" とだけ書いた見出しを置き、その後に2〜3文・120字以内で具体的な改善提案を書く。' +
      "「継続すべきこと」「改善すべきこと」「中止すべきこと」「次に試すべきこと」のうち、データから妥当なものを1〜2個だけ選んで簡潔に述べる。" +
      "抽象論ではなく、キャンペーン名や年齢層など具体的な固有名詞を使うこと。120字を超えそうな場合は文を削って必ず収めること。"
  );
  lines.push("");
  lines.push("文字数はレポートのレイアウトの都合で厳守してください。合計でも300字を超えないようにしてください。");
