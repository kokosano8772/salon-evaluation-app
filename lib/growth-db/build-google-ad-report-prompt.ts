...
export function buildGoogleAdReportAnalysisPrompt(
  ...
  lines.push("");
  lines.push(
    "1部目（見出し不要）: 2〜3文・100字以内で、前月比較を踏まえた今月の運用状況の要約を書く。良かった点・気になる点の両方に触れる。"
  );
  lines.push("");
  lines.push(
  ...
  lines.push("");
  lines.push(
    '3部目: 1行目に "## サロン様へのご提案" とだけ書いた見出しを置く。次の行（2行目）に、提案内容を一言で表す太字見出しを' +
      "25字以内で書く（体言止めではなく「〜してください」調の短い一文。例:「引き続き、メニュー内容をご確認ください」）。" +
      "3行目以降（1行空けてよい）に、2〜3文・150字以内で具体的な改善提案の本文を書く。" +
  ...
  lines.push("");
  lines.push(
    '4部目: 1行目に "## ココデザインが行う改善策" とだけ書いた見出しを置く。次の行（2行目）に、改善策を一言で表す太字見出しを' +
      "25字以内で書く（体言止めではなく「〜します」調の短い一文。例:「メニューの対象が分かりやすくなるよう調整します」）。" +
  ...
  lines.push("");
  lines.push(
    "各部の文字数はサロン「oasis-un」の実物レポート（1部目約50字・2部目約65字・見出し約20字・本文約145字/約190字）を" +
      "基準にした上限です。レイアウトの都合で厳守してください。上限を超えそうな場合は必ず文を削って収めてください。"
  );
