...
export function buildGoogleAdReportAnalysisPrompt(
  ...
  lines.push("");
  lines.push(
    '3部目: 1行目に "## サロン様へのご提案" とだけ書いた見出しを置き、その後に2〜3文・150字以内で具体的な改善提案を書く。' +
  ...
  lines.push("");
  lines.push(
    '4部目: 1行目に "## ココデザインが行う改善策" とだけ書いた見出しを置き、その後に3〜4文・200字以内で、' +
      "代理店（ココデザイン）側が来月行う運用上の改善アクションを、渡されたデータ（内訳・検索語句・年代別など）を根拠にして具体的に書く" +
      "（例:「反応の良い〇〇歳への配信比重を調整いたします」「〇〇という検索語句に合わせた内容を確認してまいります」）。"
  );
  lines.push("");
  lines.push(
    "各部の文字数はサロン「oasis-un」の実物レポート（1部目約50字・2部目約65字・3部目約145字・4部目約190字）を基準にした" +
      "上限です。レイアウトの都合で厳守してください。上限を超えそうな場合は必ず文を削って収めてください。"
  );
