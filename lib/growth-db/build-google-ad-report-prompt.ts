...
export function buildGoogleAdReportAnalysisPrompt(
  ...
  lines.push(
    "文体は次のトーンを厳守すること（美容室オーナーが安心して読める、丁寧な報告文にするため）:\n" +
      ...
      "・出力は指定された4部構成の本文のみとすること。URLを確認した過程の説明、検討メモ、チェックリスト、英語での思考過程、" +
      "「確認しました」のような報告、前置きは一切書かないこと。1文字目から直接 \"## 運用状況\" の見出しで書き始めること。"
  );
  lines.push("");
  lines.push(
    '1部目: 1行目に "## 運用状況" とだけ書いた見出しを置き、2行目に1文、どうしても足りない場合のみ2文で、合計50字程度・絶対に60字を超えないように、' +
      "前月比較を踏まえた今月の運用状況の要約を書く。良かった点・気になる点の両方に触れてよいが、1文に収まらないなら" +
      "気になる点は省略してよい。文字数を優先し、内容を削ってでも必ず60字以内に収めること。" +
      "気になる点（減少・悪化）に触れる場合は必ず先に述べ、文末は良かった点・安定している点で締めくくること。" +
      "文末をネガティブな内容（「減少しました」「悪化しました」等）で終わらせてはならない。" +
  ...
  lines.push(
    '3部目: 1行目に "## サロン様へのご提案" とだけ書いた見出しを置く。2行目に、提案内容を一言で表す太字見出しを' +
      ...
      "各アクションは10〜14字程度の体言止めで、文字数を優先し内容を削ってでも絶対に16字を超えないこと" +
      "（例:「- 変更後メニューの予約状況確認」「- 今後の掲載内容の見直し」）。" +
      (hasStorePage
        ? (isRecruitment
