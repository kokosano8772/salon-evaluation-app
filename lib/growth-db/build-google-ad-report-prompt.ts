...
export function buildGoogleAdReportAnalysisPrompt(
  ...
  lines.push("");
  lines.push(
    "1部目（見出し不要）: 3〜4文・200字以内で、前月比較を踏まえた今月の運用状況の要約を書く。良かった点・気になる点の両方に触れる。"
  );
  lines.push("");
  lines.push(
    `2部目: 1行目に "## 年代別の傾向" とだけ書いた見出しを置き、その後に1〜2文・80字以内で、年代別データのうち上記で渡した` +
      "「最も反応が良い年代」の事実だけを使って一言添える。他の年代について触れる場合も、渡された数値の範囲内に留めること。" +
      "年代別データが渡されていない場合はこの部を空欄にすること。"
  );
  lines.push("");
  lines.push(
    '3部目: 1行目に "## サロン様へのご提案" とだけ書いた見出しを置き、その後に2〜3文・160字以内で具体的な改善提案を書く。' +
  ...
  lines.push("");
  lines.push(
    '4部目: 1行目に "## ココデザインが行う改善策" とだけ書いた見出しを置き、その後に1〜2文・100字以内で、' +
      "代理店（ココデザイン）側が来月行う運用上の改善アクションを書く（例:「反応の良い年代への配信比重を調整いたします」）。"
  );
  lines.push("");
  lines.push("文字数はレポートのレイアウトの都合で厳守してください。各部の文字数上限を超えそうな場合は必ず文を削って収めてください。");
